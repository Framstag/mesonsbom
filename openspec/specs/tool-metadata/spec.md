...
### Requirement: Tool ordering in tools.components array
The entries in `metadata.tools.components` SHALL appear in this order: first `mesonsbom`, second `meson` (when `meson-info.json` is present and readable), and third the Meson backend build tool (when `intro-buildoptions.json` indicates a known backend). Any future tools SHALL appear after the backend entry.
- **WHEN** an SBOM is generated
